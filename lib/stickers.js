...
/** 稳定 subject 只负责把关系账映射回本轮 catalog id；标签不参与唯一身份。 */
export function stickerIdFromSubject(subject) {
  const value = String(subject ?? "").trim();
  const source = /^source:([^:]{1,120})$/u.exec(value);
  if (source) return source[1];
  const local = /^local:([^:]{1,120}):([a-f0-9]{64})$/iu.exec(value);
  return local ? local[1] : "";
}

/** 挑图与发送的默认规矩。 */
export function resolveStickerPolicy({ guides = [], userGuides = [], partnerGuides = [], context = {}, runtime = {}, now = new Date() } = {}) {
  const entries = effectiveClaimEntries({
    ...
    capability: "sticker.permission",
  });
  const feedbackBlocked = new Set((runtime.negativeStickerSubjects ?? []).map(String));
  const allowEntries = entries.filter(({ claim }) => claim.effect === "allow" && claim.value === "specific" && !feedbackBlocked.has(String(claim.subject ?? "")));
  const denyEntries = entries.filter(({ claim }) => claim.effect === "deny");
  const globalDenied = denyEntries.some(({ claim }) => !claim.subject || claim.value === "none");
  const allowedIds = allowEntries.map(({ claim }) => stickerIdFromSubject(claim.subject)).filter(Boolean);
  const deniedIds = [
    ...denyEntries.map(({ claim }) => stickerIdFromSubject(claim.subject)),
    ...[...feedbackBlocked].map(stickerIdFromSubject),
  ].filter(Boolean);
  return {
    allowed: !globalDenied,
    restrictToSpecific: allowEntries.length > 0,
    allowedIds: [...new Set(allowedIds)],
    deniedIds: [...new Set(deniedIds)],
    hardBlocked: globalDenied,
    guideIds: [...new Set(entries.map(({ guide }) => guide.id).filter(Boolean))],
  };
}

export function applyStickerPolicy(catalog, policy = null) {
  if (!catalog || !policy) return catalog;
  if (policy.allowed === false) return { ...catalog, stickers: [], byId: new Map(), preferred: new Set() };
  const denied = new Set((policy.deniedIds ?? []).map(String));
  const allowed = new Set((policy.allowedIds ?? []).map(String));
  const stickers = (catalog.stickers ?? []).filter((row) => {
    const id = String(row?.id ?? "");
    if (denied.has(id)) return false;
    return policy.restrictToSpecific ? allowed.has(id) : true;
  });
  return { ...catalog, stickers, byId: new Map(stickers.map((row) => [String(row.id), row])), preferred: new Set([...(catalog.preferred ?? [])].filter((id) => stickers.some((row) => String(row.id) === String(id)))) };
}
 ...
 */
export function pickSticker(catalog, { keyword, recentIds = [], rules = DEFAULT_STICKER_RULES, rnd = Math.random, policy = null } = {}) {
  const usable = applyStickerPolicy(catalog, policy);
  if (policy?.allowed === false) return null;
  // 两级：先精确/包含匹配，一个都没中再退到共用汉字的弱匹配。
  // 弱匹配也没捞着，那才是真没图可发，这次就不发。
  let matched = matchStickers(usable?.stickers, keyword);
  if (matched.length === 0) matched = looseMatchStickers(usable?.stickers, keyword);
