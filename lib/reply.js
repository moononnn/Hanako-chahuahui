...
export const DEFERRED_NOTICE_MIN_MS = 15 * MIN;
export const DEFERRED_NOTICE_MAX_MS = 45 * MIN;
 ...
 * @param {{wakeDecision?: "normal"|"exception"|"deferred", hardDelayCapMs?: number}} [input.sleepPolicy]
 * @returns {{ mode: "hand"|"away"|"dozing", delayMs: number, kind?: string|null, phone: object }}
 *          `phone` 是推进后的新状态，调用方负责存回去。
 */
export function planReply({ phone = null, now = new Date(), sleep = null, rnd = Math.random, sleepPolicy = null }) {
  ...
  if (dozing.dozing) {
    const wakeDecision = ["normal", "exception", "deferred"].includes(sleepPolicy?.wakeDecision)
      ? sleepPolicy.wakeDecision
      : "normal";
    const regularDelay = Math.round(DOZE_NOTICE_MIN_MS + rnd() * (DOZE_NOTICE_MAX_MS - DOZE_NOTICE_MIN_MS));
    const deferredDelay = Math.round(DEFERRED_NOTICE_MIN_MS + rnd() * (DEFERRED_NOTICE_MAX_MS - DEFERRED_NOTICE_MIN_MS));
    const delayMs = wakeDecision === "deferred"
      ? Math.min(Math.max(1, Number(sleepPolicy?.hardDelayCapMs) || DEFERRED_NOTICE_MAX_MS), deferredDelay)
      : regularDelay;
    return {
      mode: "dozing",
      delayMs,
