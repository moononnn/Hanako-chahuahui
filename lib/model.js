 ...
 */
export async function askUtility(ctx, { systemPrompt, userText, maxTokens = 200, temperature = 0.4 }) {
  const requestId = rid("utility");
  const result = await ctx.models.utility({
    requestId,
    scope: "app",
    systemPrompt,
...
export async function generateReply(ctx, { systemPrompt, messages, diagnostics, maxTokens = 900, modelRef = null, catalog = null, onRaw = null, temperature = 0.95 }) {
  ...
  try {
    ...
    if (chosen) {
      ...
      const raw = await readBody(response, diagnostics);
      // 内测对账用：把原始流交出去，看正文里的怪字符到底是 provider 发的，还是我们解析时吞的
      onRaw?.(raw);
    ...
    diagnostics?.({ event: "models.stream.failed", error: error?.message ?? String(error) });
  } finally {
  ...
  const utility = await ctx.models.utility({
