 ...
 */
export function proactiveSpec({ partnerName, userName, topic, hobby, memoryText, relationNote, adaptationText, searchContext, stickerText, currentTimeText, followup, wakeEcho, sceneEcho, contextText, userRhythmText }) {
  ...
  const blocks = [
    ...
    wakeEcho
      ...
      : sceneEcho
        ? [
            ...
            block("她上一轮说的话（只作背景参考）：", sceneEcho.userText),
            block("你上一轮回她的话（只作背景参考）：", sceneEcho.assistantText),
          ].filter(Boolean).join("\n")
        : "",
    ...
    followup?.read
      ...
      : "如果有共同话题，围绕它带来一点你自己的新东西；如果没有，就从你的兴趣里自然分享一个想法、疑问或小发现。最近场景只是背景，除非确实有未完的情绪、照顾事项或约定，否则不要为了衔接而回顾上一轮。不要列爱好清单，也不要编造现实经历。",
...
/** 半夜睡不着的那种留言。 */
export function nightSpec({ partnerName, userName, memoryText, worry, correction, relationNote, adaptationText, currentTimeText }) {
  const blocks = [
