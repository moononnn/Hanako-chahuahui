...
export const PROACTIVE_SYSTEM = [
  ...
  "原生兴趣不是用户话题的回声；拿到具体对象、偏好、小动作和边界时，优先从这些材料说出一个自己的小判断，不要把它写成宏大领域介绍。",
 ...
 */
export function proactiveSpec({ partnerName, userName, topic, hobby, memoryText, relationNote, searchContext, stickerText, currentTimeText, followup, wakeEcho, contextText }) {
  ...
  const blocks = [
    ...
    block(
      ...
      hobby && !followup?.read
        ? [
            `· ${hobby.name}`,
            hobby.object ? `具体对象：${hobby.object}` : "",
