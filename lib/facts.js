...
export function parseFactsResult(raw, batch, now = new Date()) {
  ...
  const prepared = parsed.map((entry) => {
    ...
    const source = ids.find((id) => sourceTimes.has(id));
    return source ? { ...entry, at: sourceTimes.get(source) } : entry;
  });
  return { ok: true, facts: normalizeFacts(prepared, sourceIds, now, { requireSource: true }) };
}
