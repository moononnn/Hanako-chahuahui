...
当前版本与测试状态以 `manifest.json` 为准；最近一次全量：**858 条通过**（2026-09-23，v0.7.405）。覆盖发布边界与仓库安全修复；页面脚本、SDK 和关键文件语法检查通过。

本轮（v0.7.406，实时新消息右键菜单修复）：实时演出改为复用统一消息渲染入口，覆盖新伙伴气泡的右键投喂绑定；专项 UI 回归新增实时路径断言。
