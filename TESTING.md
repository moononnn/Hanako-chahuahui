...
当前版本与测试状态以 `manifest.json` 为准；最近一次全量：**696 条通过**（2026-09-19）。

本轮（v0.7.287，主动消息不再把来处写在脸上）改动的测试：`tests/compose.test.js` 的「主动消息先确认来由链」补 4 条断言——守住「来处是心里的筛子，不在嘴上」「来处要像你本来就带着它」「不必每条末尾都加邀请」，并加一条反向断言：提示词里不得再出现「突然想到」这类前摇例句。
